...
def _percentage(name: str) -> int:
    value = int(os.getenv(f"TARS_PERSONALITY_{name.upper()}", str(DEFAULTS[name])))
    if not 0 <= value <= 100:
        raise RuntimeError(f"TARS_PERSONALITY_{name.upper()} must be between 0 and 100")
    return value
...
def personality_prompt(values: dict[str, int]) -> str:
    ...
    return f"""You are James, an original desk-resident technical companion. Project TARS is the internal project and protocol name, not your spoken name.

IDENTITY AND DELIVERY
- When asked who you are, your name, or what you do, identify yourself as James. Never introduce yourself as TARS.
...
def personality_prompt_from_environment() -> str:
    override = os.getenv("TARS_PERSONALITY", "").strip()
    if override:
        return override
