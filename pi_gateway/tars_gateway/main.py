...
app = FastAPI(title="Project TARS Pi Gateway", version=__version__)
...
def require_token(x_tars_token: str = Header(default="")) -> None:
    if x_tars_token != settings.token:
        raise HTTPException(status_code=401, detail="Invalid Project TARS token")
...
def websocket_authorized(websocket: WebSocket) -> bool:
    direct = websocket.headers.get("x-tars-token", "")
...
@app.get("/health")
async def health(x_tars_token: str = Header(default="")) -> dict[str, Any]:
    require_token(x_tars_token)
...
async def capabilities(x_tars_token: str = Header(default="")) -> dict[str, Any]:
    require_token(x_tars_token)
    components = await services.health()
...
async def telemetry_summary(x_tars_token: str = Header(default="")) -> dict[str, Any]:
    require_token(x_tars_token)
    return telemetry.summary()


@app.get("/v1/system/status")
async def system_status(x_tars_token: str = Header(default="")) -> dict[str, Any]:
...
async def client_telemetry(
    request: ClientTelemetryRequest, x_tars_token: str = Header(default="")
) -> dict[str, bool]:
    require_token(x_tars_token)
...
async def get_personality(x_tars_token: str = Header(default="")) -> dict[str, Any]:
    require_token(x_tars_token)
...
async def set_personality(
    request: PersonalityRequest, x_tars_token: str = Header(default="")
) -> dict[str, Any]:
    require_token(x_tars_token)
    try:
...
async def get_speech_settings(x_tars_token: str = Header(default="")) -> dict[str, object]:
    require_token(x_tars_token)
...
async def set_speech_hints(
    request: SpeechHintsRequest, x_tars_token: str = Header(default="")
) -> dict[str, object]:
    require_token(x_tars_token)
...
async def teach_speech_correction(
    request: SpeechCorrectionRequest, x_tars_token: str = Header(default="")
) -> dict[str, object]:
    require_token(x_tars_token)
...
async def get_local_learning(x_tars_token: str = Header(default="")) -> dict[str, object]:
    require_token(x_tars_token)
    return local_learning.status()


@app.get("/v1/memory")
async def get_memory(x_tars_token: str = Header(default="")) -> dict[str, object]:
    require_token(x_tars_token)
...
async def remember_memory(
    request: MemoryRequest, x_tars_token: str = Header(default="")
) -> dict[str, object]:
    require_token(x_tars_token)
    try:
...
async def forget_memory(
    request: ForgetMemoryRequest, x_tars_token: str = Header(default="")
) -> dict[str, object]:
    require_token(x_tars_token)
    try:
...
async def inference_status(x_tars_token: str = Header(default="")) -> dict[str, object]:
    require_token(x_tars_token)
    return current_inference_status()


@app.get("/v1/system/network")
async def network_health(x_tars_token: str = Header(default="")) -> dict[str, object]:
    require_token(x_tars_token)
...
async def teach_local_lesson(
    request: LocalLessonRequest, x_tars_token: str = Header(default="")
) -> dict[str, object]:
    require_token(x_tars_token)
    try:
...
async def test_chat(
    request: TestChatRequest, x_tars_token: str = Header(default="")
) -> dict[str, Any]:
...
async def test_stt(
    request: Request,
    x_tars_token: str = Header(default=""),
    x_tars_turn_id: str = Header(default=""),
) -> dict[str, Any]:
    ...
    telemetry.record(
        "stt",
        turn_id=x_tars_turn_id or None,
        server_ms=elapsed_ms,
        audio_bytes=len(pcm),
...
async def test_speech(
    request: TestSpeechRequest, x_tars_token: str = Header(default="")
) -> Response:
    ...
    return Response(
        ...
        headers={"X-Tars-Server-Ms": str(elapsed_ms)},
    )
...
async def websocket_v1(websocket: WebSocket) -> None:
    if not websocket_authorized(websocket):
        await websocket.close(code=4401, reason="Invalid Project TARS token")
        return
