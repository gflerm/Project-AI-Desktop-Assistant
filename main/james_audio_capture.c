...
#define JAMES_AUDIO_REPORT_FRAMES (1000U / JAMES_AUDIO_FRAME_MS)
#define JAMES_AUDIO_CAPTURE_CORE 1
#define JAMES_AUDIO_CAPTURE_PRIORITY 20
#define JAMES_AUDIO_CAPTURE_STACK_BYTES 4096
#define JAMES_AUDIO_CLIP_LEVEL 32760

static const char *TAG = "james_audio";

typedef struct {
    esp_codec_dev_handle_t input_codec;
    esp_codec_dev_handle_t output_codec;
    TaskHandle_t capture_task;
    james_audio_ring_t ring;
    volatile bool playback_active;
    uint64_t read_errors;
    uint32_t next_sequence;
} james_audio_context_t;

static james_audio_context_t s_audio;

static void james_audio_capture_task(void *argument)
{
    james_audio_context_t *context = (james_audio_context_t *)argument;
    james_audio_frame_t frame = {0};
    ...
    while (true) {
        if (context->playback_active) {
            vTaskDelay(pdMS_TO_TICKS(JAMES_AUDIO_FRAME_MS));
            continue;
        }
        ...
        if (!james_audio_ring_push_latest(&context->ring, &frame)) {
            ESP_LOGE(TAG, "Audio queue rejected frame sequence=%" PRIu32,
                     frame.sequence);
        }
        for (size_t index = 0; index < JAMES_AUDIO_SAMPLES_PER_FRAME; ++index) {
            ...
            clipped_samples += magnitude >= JAMES_AUDIO_CLIP_LEVEL;
            sum_squares += (uint64_t)(sample * sample);
            sample_count++;
        }
        report_frames++;

        if (report_frames >= JAMES_AUDIO_REPORT_FRAMES) {
            james_audio_ring_stats_t stats = {0};
            james_audio_ring_get_stats(&context->ring, &stats);
...
esp_err_t james_audio_capture_start(void)
{
    ...
    const i2s_std_config_t i2s_config = {
        .clk_cfg = I2S_STD_CLK_DEFAULT_CONFIG(JAMES_AUDIO_SAMPLE_RATE_HZ),
    ...
    esp_codec_dev_sample_info_t format = {
        .bits_per_sample = JAMES_AUDIO_BITS_PER_SAMPLE,
        .channel = JAMES_AUDIO_CHANNELS,
        .channel_mask = 0,
        .sample_rate = JAMES_AUDIO_SAMPLE_RATE_HZ,
        .mclk_multiple = 0,
    };
    ...
    ESP_RETURN_ON_ERROR(
        esp_codec_dev_set_in_gain(s_audio.input_codec,
                                  CONFIG_JAMES_AUDIO_MIC_GAIN_DB),
        TAG, "Microphone gain configuration failed");
    ESP_RETURN_ON_ERROR(
        esp_codec_dev_set_out_vol(s_audio.output_codec,
                                  CONFIG_JAMES_AUDIO_SPEAKER_VOLUME),
        TAG, "Speaker volume configuration failed");
    ESP_RETURN_ON_ERROR(
        james_audio_ring_init(&s_audio.ring, CONFIG_JAMES_AUDIO_RING_FRAMES),
        TAG, "Audio queue allocation failed");

    if (xTaskCreatePinnedToCore(james_audio_capture_task, "audio_rx_task",
                                JAMES_AUDIO_CAPTURE_STACK_BYTES, &s_audio,
                                JAMES_AUDIO_CAPTURE_PRIORITY,
                                &s_audio.capture_task,
                                JAMES_AUDIO_CAPTURE_CORE) != pdPASS) {
        return ESP_ERR_NO_MEM;
    }
    ESP_LOGI(TAG,
             "Audio ready: %u Hz, %u-bit mono, mic gain=%d dB, speaker=%d%%",
             JAMES_AUDIO_SAMPLE_RATE_HZ, JAMES_AUDIO_BITS_PER_SAMPLE,
             CONFIG_JAMES_AUDIO_MIC_GAIN_DB,
             CONFIG_JAMES_AUDIO_SPEAKER_VOLUME);
    return ESP_OK;
}
...
bool james_audio_capture_read(james_audio_frame_t *frame, TickType_t timeout)
{
    return s_audio.ring.queue != NULL &&
           james_audio_ring_pop(&s_audio.ring, frame, timeout);
}
...
{
    if (stats != NULL) {
        james_audio_ring_get_stats(&s_audio.ring, stats);
    }
}
