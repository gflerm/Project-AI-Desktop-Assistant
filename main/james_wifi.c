...
#define JAMES_WIFI_CONNECTED_BIT BIT0
#define JAMES_WIFI_INITIAL_TIMEOUT_MS 30000

static const char *TAG = "james_wifi";
static EventGroupHandle_t s_events;

static void james_wifi_event(void *argument, esp_event_base_t base, int32_t id,
                            void *data)
{
    (void)argument;
    if (base == WIFI_EVENT && id == WIFI_EVENT_STA_START) {
        ...
        xEventGroupClearBits(s_events, JAMES_WIFI_CONNECTED_BIT);
        ...
        xEventGroupSetBits(s_events, JAMES_WIFI_CONNECTED_BIT);
    }
}

esp_err_t james_wifi_start(void)
{
    if (JAMES_PRIVATE_WIFI_SSID[0] == '\0') {
    ...
    strlcpy((char *)config.sta.ssid, JAMES_PRIVATE_WIFI_SSID,
            sizeof(config.sta.ssid));
    strlcpy((char *)config.sta.password, JAMES_PRIVATE_WIFI_PASSWORD,
    ...
    const EventBits_t connected = xEventGroupWaitBits(
        s_events, JAMES_WIFI_CONNECTED_BIT, pdFALSE, pdTRUE,
        pdMS_TO_TICKS(JAMES_WIFI_INITIAL_TIMEOUT_MS));
    if (!(connected & JAMES_WIFI_CONNECTED_BIT)) {
        ESP_LOGE(TAG, "No IP address after %d seconds",
                 JAMES_WIFI_INITIAL_TIMEOUT_MS / 1000);
...
bool james_wifi_is_connected(void)
{
    return s_events != NULL &&
           (xEventGroupGetBits(s_events) & JAMES_WIFI_CONNECTED_BIT) != 0;
}
