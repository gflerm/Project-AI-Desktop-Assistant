...
from james_windows_tester import JamesTester
...
class WindowsTesterCaptureTests(unittest.TestCase):
    ...
    def test_private_turn_capture_writes_audio_and_metadata(self) -> None:
        tester = JamesTester.__new__(JamesTester)
        tester._personality_values = {"humour": 65}
        tester._last_turn_path = None
        pcm = bytes(640)
        with tempfile.TemporaryDirectory() as directory, patch.object(
            JamesTester, "_captures_root", return_value=Path(directory)
        ):
            tester._save_recorded_turn(
                turn_id="12345678-test",
                mode="ptt",
                prompt="Hello James",
                raw_transcript="Hello James",
                adapted_transcript="Hello James",
