# Project TARS — Teaching and Response Improvement Plan
...
TARS should **not** be fine-tuned yet. The recordings show that most current
...
The review covered all files that contain recorded TARS interactions or
...
|---|---|---|---|
| Refused or invented the current time | TARS denied clock access, invented 10:45 AM, and incorrectly described South African time | A model was asked for a deterministic host fact | Clock tool first; already implemented, retain regression tests |
...
```json
{
  ...
  "answer_feedback": {
    ...
    "preferred_answer": "the answer TARS should have given"
  },
  "expected_behaviour": {
...
- [ ] Keep phrase/word corrections only when they recur or are project names,
  such as TARS, Titanium, Ollama, Gemini and ESP32-P4.
...
- [ ] Log requested action, authorization, execution result and completion; TARS
...
| STT terminology | TARS, gateway, Titanium, Gemini, Ollama and ESP32-P4 transcribe correctly |
...
This sequence teaches TARS by turning each correction into a verified system
