#!/usr/bin/env python3
"""Native Windows push-to-talk tester for Project TARS."""
...
from tars_feedback import empty_feedback, migrate_record, update_turn as save_turn_feedback
...
class TarsTester(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("Project TARS Voice Tester — PTT and Timing")
        ...
        ttk.Label(frame, text="TARS response").grid(row=8, column=0, columnspan=2, sticky="w")
    ...
    def _run_action(self, action) -> None:
        try:
            ...
            self.after(0, lambda: messagebox.showerror("Project TARS", str(error)))
            self.after(0, lambda: self.status.set("Test failed."))

    def _load_token(self) -> None:
        def action():
            key, hosts = Path.home()/".ssh"/"id_ed25519", Path.home()/".ssh"/"known_hosts"
            command = ["ssh", "-i", str(key), "-o", "BatchMode=yes", "-o", "StrictHostKeyChecking=yes", "-o", f"UserKnownHostsFile={hosts}", self.ssh_host.get().strip(), "sudo -n sed -n 's/^TARS_TOKEN=//p' /etc/tars/tars.env"]
            token = subprocess.run(command, capture_output=True, text=True, timeout=15, check=True).stdout.strip()
            if len(token) < 24:
                raise RuntimeError("The TARS token could not be loaded over SSH.")
    ...
    def _request(
        ...
        if not token:
            raise RuntimeError("Load or paste the TARS token first.")
    ...
    def _telemetry_summary(self) -> None:
        def action():
            ...
            self.after(0, lambda: self._show_json_window("TARS Telemetry Summary", summary))
    ...
    def _show_personality_window(self, values) -> None:
        ...
        window.title("TARS Personality — live controls")
    ...
    def _show_speech_window(self, settings) -> None:
        window = tk.Toplevel(self)
        window.title("TARS STT learning")
    ...
    def _teach_stt(self) -> None:
        ...
        if not observed or not turn_id or not path:
            messagebox.showinfo(
                "Project TARS", "Record one PTT utterance first, then edit its transcript here."
            )
            return
        if not corrected or corrected == observed:
            messagebox.showinfo(
                "Project TARS", "Edit the transcript to what you actually said, then teach it."
            )
            return
    ...
    def _chat_and_speak(self, prompt, turn_id):
        ...
        return result, {
            ...
            "tts_server_ms": int(headers.get("x-tars-server-ms", 0)),
    ...
    def _ask(self, prompt) -> None:
        ...
        self._background("TARS is preparing a response...", action)

    def _ptt_start(self, _event=None) -> None:
        if self._input_stream is not None:
            return
        if sd is None:
            messagebox.showerror("Project TARS", "Microphone support needs sounddevice. Use Launch-TARS-Tester.ps1.")
            return
        self._recorded_chunks = []
        ...
        try:
            ...
            self._input_stream = None
            messagebox.showerror("Project TARS", f"Could not open the microphone: {error}")
    ...
    def _captures_root() -> Path:
        return Path(__file__).resolve().parents[1] / "captures" / "tars-sessions"
    ...
    def _flag_shortcomings(self) -> None:
        ...
        if not turn_id:
            messagebox.showinfo(
                "Project TARS", "Complete a recorded turn before flagging its shortcomings."
            )
            return
        window = tk.Toplevel(self)
        window.title("Flag TARS shortcomings")
    ...
    def _analyze_recordings(self) -> None:
        def action():
            script = Path(__file__).with_name("analyze_tars_sessions.py")
    ...
    def _play_wav(self, wav) -> None:
        self._stop_audio()
        with tempfile.NamedTemporaryFile(prefix="tars-response-", suffix=".wav", delete=False) as output:
