#!/usr/bin/env python3
"""Replay only explicitly approved private TARS regression cases."""
...
QUEUE = ROOT / "captures" / "tars-review-queue.json"
OUTPUT = ROOT / "captures" / "tars-regression-results.json"
...
def main() -> None:
    ...
    if len(token) < 24:
        raise SystemExit("TARS_TOKEN is required for --run")
    base = os.environ.get("TARS_GATEWAY_URL", "http://192.168.8.107:8090").rstrip("/")
    results = []
